...
class _State extends State<ChatRoomScreen> {
  ...
  void initState() {
    ...
    // ── PERBAIKAN: beri tahu ChatService bahwa user ini sedang aktif di room ──
    // Sehingga pesan yang masuk TIDAK menghasilkan push notification (sudah
    // terlihat langsung di layar). Kalau tidak diset, setiap pesan yang dikirim
    // lawan akan memunculkan notif meski kamu sedang buka chat yang sama.
    ChatService.setActiveRoom(_roomId, _myUid);

    // Tandai semua pesan sebagai sudah dibaca saat masuk ke room
    _chat.tandaiDibaca(_roomId, _myUid);
  }

  @override
  void dispose() {
    // ── PERBAIKAN: hapus dari tracker saat keluar dari halaman chat ──
    // Setelah ini, pesan baru dari lawan akan kembali memunculkan push notif.
  ...
  void _pickImg() => showModalBottomSheet(
      context: context,
      builder: (_) => SafeArea(
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ListTile(
                ...
                onTap: () async {
                  ...
                  if (f != null) {
                    setState(() => _sending = true);
                    final url = await _storage.uploadBuktiBayar(
                        'chat_${DateTime.now().millisecondsSinceEpoch}', f);
                    _send(text: '[Gambar]', fileUrl: url);
                  }
                },
              ),
              ListTile(
                          ...
                          Flexible(
                            child: Column(
                              ...
                              children: [
                                ...
                                if (m.text.isNotEmpty && m.text != '[Gambar]')
                                  Container(
