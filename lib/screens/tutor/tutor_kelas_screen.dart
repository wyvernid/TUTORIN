      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Padding(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            PopupMenuButton<String>(onSelected: (v) { if (v == 'edit') Navigator.push(context, MaterialPageRoute(builder: (_) => const TutorTambahKelasScreen())); else if (v == 'off') widget.service.nonaktifkan(k.id); },
              itemBuilder: (_) => [const PopupMenuItem(value: 'edit', child: Text('Edit')), const PopupMenuItem(value: 'off', child: Text('Nonaktifkan'))],
        ...
        if (_expanded) StreamBuilder<List<BookingModel>>(
          ...
          builder: (_, snap) {
            ...
            final murid = snap.data!.where((b) => b.kelasId == k.id && b.status != 'rejected').toList();
            ...
            return Column(children: murid.map((b) => Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
              ...
              child: Row(children: [
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(b.studentNama, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600)), Text(b.jadwalDipilih, style: TextStyle(fontSize: 10, color: Colors.grey[500]))])),
                Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 3), decoration: BoxDecoration(color: b.status == 'confirmed' ? Colors.green[50] : Colors.orange[50], borderRadius: BorderRadius.circular(20)),
                  child: Text(b.status == 'confirmed' ? 'Confirmed' : 'Pending', style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: b.status == 'confirmed' ? Colors.green[700] : Colors.orange[700]))),
