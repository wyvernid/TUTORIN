...
//   final _umur = TextEditingController(), _pass = TextEditingController(), _confirm = TextEditingController();
//   bool _op = true, _oc = true, _loading = false;
//   String? _error;
//   final _auth = AuthService();

//   void _daftar() async {
//     if (_pass.text != _confirm.text) { setState(() => _error = 'Password tidak sama'); return; }
//     setState(() { _loading = true; _error = null; });
//     try {
//       await _auth.register(email: _email.text.trim(), password: _pass.text,
//         nama: _nama.text.trim(), role: 'student', usia: int.tryParse(_umur.text));
//       if (!mounted) return;
...
//       const Text('REGISTER\nSTUDENT', textAlign: TextAlign.center,
...
//           _field('Email', _email, hint: 'student@example.com', icon: Icons.email_outlined),
...
//           const SizedBox(height: 12),
//           _field('Password', _pass, icon: Icons.lock_outline, obscure: _op, toggle: () => setState(() => _op = !_op)),
//           const SizedBox(height: 12),
//           _field('Konfirmasi Password', _confirm, icon: Icons.lock_outline, obscure: _oc, toggle: () => setState(() => _oc = !_oc)),
...
//                 : const Text('DAFTAR', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 15, letterSpacing: 2)))),
//         ])),
//       const SizedBox(height: 30),
//     ]))));

//   Widget _field(String label, TextEditingController ctrl, {String? hint, IconData? icon, bool obscure = false, VoidCallback? toggle, TextInputType? type}) =>
//     Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
//       Text(label, style: const TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w600)),
//       const SizedBox(height: 6),
//       TextField(controller: ctrl, obscureText: obscure, keyboardType: type, style: const TextStyle(fontSize: 14),
//         decoration: InputDecoration(hintText: hint, hintStyle: TextStyle(color: Colors.grey[400], fontSize: 13),
...
//           suffixIcon: toggle != null ? IconButton(icon: Icon(obscure ? Icons.visibility_off_outlined : Icons.visibility_outlined, color: Colors.grey[500], size: 20), onPressed: toggle) : null)),
//     ]);
// }
...
class _State extends State<RegisterStudentScreen> {
  ...
  void _daftar() async {
    ...
    try {
      ...
      // Tampilkan pesan Firebase yang lebih user-friendly
