...
class _State extends State<StudentProfilScreen> {
  ...
  // ── DEBUG SEMENTARA: tes push notification ke diri sendiri ──
  // Kirim push lewat OneSignal ke uid akun yang SEDANG login saat ini,
  // tanpa perlu logout/ganti akun. Cocok untuk tes dengan 1 HP saja.
  // HAPUS fungsi & tombolnya (lihat actions di AppBar) sebelum deploy final.
  Future<void> _tesKirimPushKeDiriSendiri() async {
    final uid = FirebaseAuth.instance.currentUser?.uid;
    if (uid == null) return;
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Push dikirim! Minimize app SEKARANG untuk cek tray.')),
      );
    }
    await OneSignalService.kirimPush(
      targetUid: uid,
      judul: 'Tes Push Notifikasi',
      pesan: 'Kalau ini muncul di tray, berarti push sudah jalan!',
    );
  }
  ...
  Widget build(BuildContext context) {
    final u = _user;
    return Scaffold(backgroundColor: const Color(0xFFF5F7FA),
      appBar: AppBar(title: const Text('Profil'), automaticallyImplyLeading: false, actions: [
        // ── DEBUG SEMENTARA: tombol tes push notification ──
        // HAPUS IconButton ini sebelum submit/deploy final.
        IconButton(
          icon: const Icon(Icons.bug_report_rounded),
          tooltip: 'Tes Push Notif (debug)',
          onPressed: _tesKirimPushKeDiriSendiri,
        ),
        IconButton(icon: const Icon(Icons.edit_rounded), onPressed: _showEditSheet),
      ]),
      body: u == null ? const Center(child: CircularProgressIndicator()) : SingleChildScrollView(child: Column(children: [
        Container(width: double.infinity, padding: const EdgeInsets.fromLTRB(20,28,20,24),
          decoration: const BoxDecoration(color: Color(0xFF1565C0), borderRadius: BorderRadius.vertical(bottom: Radius.circular(28))),
